...
def find_pairs_com(array, k)
  # array.combination(2).to_a.reject {|pair| pair.sum !=k }
  array.combination(2).to_a.select {|pair| pair.sum ==k }
  # array.combination(2).to_a.reduce([]){|result, pair| result << pair if pair.sum == k; result }
end
