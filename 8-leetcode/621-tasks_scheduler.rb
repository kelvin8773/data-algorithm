...
def least_interval(tasks, n)
  ...
  task_hash = Hash[task_hash.sort_by { |key, val| -val }]

  while tasks.size > 0
    task_hash = task_hash.select { |key, val| val > 0 }

    if task_hash.keys.size > n 
      for i in 0..n do 
        key = task_hash.keys[i]
        process << key
        tasks.pop
        task_hash[key] -= 1
      end
    else 
      task_hash.keys.each do |x|
        process << x
        tasks.pop
        task_hash[x] -= 1
      end
      remain = (n + 1) - task_hash.keys.size
      if remain > 0 && tasks.size > 0
        remain.times { process << "idle"}
      end
    end
  end
